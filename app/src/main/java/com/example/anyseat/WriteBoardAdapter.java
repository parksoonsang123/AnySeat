...
public class WriteBoardAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private final List<WriteBoardItem> mDataList;
    Context context;
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if(holder instanceof FirstViewHolder){  // 선택한 이미지들
            //((FirstViewHolder)holder).imageView.setImageURI(mDataList.get(position).getImageuri());
        ...
        else if(holder instanceof SecondViewHolder){    // + 이미지
            //((SecondViewHolder)holder).imageView.setImageURI(mDataList.get(position).getImageuri());
        }
    }
